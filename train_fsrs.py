...
def train_epoch(args, TrainImgLoader, model, model_fn, optimizer, epoch, iters, lr_scheduler, mmds):
    ...
    for batch_idx, data in enumerate(tbar):
        loss, recon_loss, color_loss, detail_loss = model_fn(args, data, model, iters, mmds)
        loss = loss.mean()
        # backward and update
        optimizer.zero_grad()
        loss.backward()
...
def main():
    ...
    mmds_checkpoint = torch.load('out_of_mmds', map_location='cuda:0')['state_dict']
    mmds.load_state_dict(mmds_checkpoint)

    for param in mmds.parameters():
        param.requires_grad = False

    fsrs = FSRS()
    fsrs = torch.nn.DataParallel(fsrs, device_ids=device_ids)
    fsrs = fsrs.cuda(device=device_ids[0])

    # create optimizer
    optimizer = torch.optim.RMSprop([{"params": fsrs.module.ll_encoder.parameters(), "lr": 2e-4},
                                     {"params": fsrs.module.ll_decoder.parameters(), "lr": 2e-4},
                                     ])
    ...
    #Loss in FSRS
    loss_fn = Loss_fsrs(device = device_ids[0])
    ...
    model_fn = model_fn_decorator(loss_fn=loss_fn, device=device_ids[0], mode='fsrs')
    ...
    for epoch in range(args.LOAD_EPOCH + 1, args.EPOCHS + 1):
        learning_rate, avg_train_loss, iters = train_epoch(args, TrainImgLoader, fsrs, model_fn, optimizer, epoch,
                                                           iters, lr_scheduler, mmds)
